...
namespace TestProject1
{
    ...
    ///
    /// 基礎梁は端点を 3D で縮めてから描き、杭は帯を 2D で縮めます。投影がアフィンなので
    /// どちらで縮めても同じ結果になりますが、<b>縮める割合が食い違うと梁と杭で
    /// 隙間の大きさが変わり、同じ図の中でちぐはぐに見えます。</b>
    ///
    /// 縮めるのは<b>要素だけ</b>です。節点・杭先端・節杭の節は本当の位置に残します。
    ...
    public class ShrinkElementModeTests
    {
        ...
        /// 3D 側 (梁) と 2D 側 (杭) で、0.8 のとき中央 80% が残る。
        /// </summary>
        [TestMethod]
        public void TheShrink_KeepsTheMiddleAndIsSymmetric()
        {
            // 2D 側の式をそのまま再現する (ShrinkBand2D は private)
        ...
        public void ThePileBand_FollowsTheMode()
        {
            ...
            Assert.IsTrue(src.Contains("ShrinkBand2D"),
                "杭の帯を縮める処理がありません");
        ...
        public void ThePileToe_IsNotShrunk()
        {
            ...
            Assert.IsFalse(before.Contains("ShrinkBand2D"),
