...
namespace TestProject1
{
    ...
    public class SettlementInvariantTests
    {
        ...
        /// 厳密には離散化誤差ぶん動く。<b>実測は設計例集3.1 で 1.02%
        /// (0.9701 → 0.9800 mm)、他の例題は 0.3% 未満</b>である（杭区間 9→16 / 5→8）。
        /// 水平解析が要素分割で 10〜27% 動くのとは対照的で、沈下は頑健である。</para>
        ///
        /// <para>だから 2% を上限に置く。実測の 2 倍の余裕があり、
        /// 層と杭区間の対応付けを取り違える類の誤りはこれを軽々と超える。
        /// 上限は以前 1% (実測 0.32% の 3 倍) だった。2026-09-12 に例題ビルダーを実機の
        /// 読込へ寄せて<b>根入れと ΔZc が例題どおりに入った</b>ため、設計例集3.1 の
        /// 杭区間の切り方が変わり、離散化誤差が 1.02% に増えた。</para>
        /// </summary>
        [DataTestMethod]
        [DataRow("Example9", "PileExample9")]
        [DataRow("Example3_1", "PileExample3_1")]
