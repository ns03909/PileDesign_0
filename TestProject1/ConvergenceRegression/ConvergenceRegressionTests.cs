...
namespace TestProject1
{
    ...
    public class ConvergenceRegressionTests
    {
        ...
        public void ResetSharedState()
        {
            ...
            //
            // 2026-08-21 の調査結果 (Example10 / L2-1.C1.Liq の反復数):
            //     温キャッシュ (クリアしない)      181  ← スナップショットと一致
            //     冷キャッシュ (毎テスト前クリア)  353
            //     キャッシュ完全バイパス           181
            // 値がキーの純粋な関数なら 冷 == バイパス になるはずだが、そうなっていない。
            // キーの取りこぼしは検証済みで、バイパス実行中に同一キーから異なる曲線が
            // 生成された例は 1 件も無かった (曲線全体のハッシュで確認)。
            // つまり「キャッシュを使うか否か」で解析結果が変わる経路がまだ残っている。
            //
            // 未解明のため、ここでクリアはしない (クリアすると本来見たい退化ではない差で落ちる)。
            // 根本原因は別途調査する。詳細は memory の
            // project_mphi_cache_nondeterminism を参照。
        }
