...
namespace TestProject1
{
    ...
    public class ConvergenceRegressionTests
    {
        ...
        public void ResetSharedState()
        {
            ...
            //
            // 経緯: 以前は「クリアすると反復数が 181 → 353 に跳ねる」ため、あえてクリアしていなかった。
            // 2026-08-21 の調査で、この跳ねは M-φ キャッシュとは無関係と判明した。
            // 真因は AnaModel.MapOnKmat の全体剛性組立で、beam を ConcurrentBag に集めた
            // thread-local COO リストから加算しており、加算順が実行ごとに変わっていたこと。
            // K が ULP レベルで揺れ、非線形 NR の分岐が変わって反復数が 175 ⇄ 353 に振れていた
            // (キャッシュの温冷はスレッドのタイミングを変えるので相関して見えていただけ)。
            // 組立順を beam のインデックス順に固定したので、同一入力なら結果はビット一致する。
