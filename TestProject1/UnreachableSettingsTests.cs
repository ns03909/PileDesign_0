...
namespace TestProject1
{
    ...
    /// 実装も引き渡しも済んでいるのに、画面に選択肢が無いため
    /// <c>SelectedAnalysisMode</c> は常に荷重制御のままで、
    /// <c>TryDisplacementControlConvergence</c> は実アプリで一度も通らない。</para>
    ///
    ...
    public class UnreachableSettingsTests
    {
        ...
        {
            ["AnalysisModes"] =
                "単杭沈下の変位制御法。実装済みだが画面に選択肢が無く、常に荷重制御で走る。"
                + "2026-09-11 に実際に走らせたところ、そのままでは使えない状態だった —— "
                + "(1) 引張側 (浮き上がり) が出ない (61 点すべて沈下が正、最小 +0.45mm。"
                + "荷重制御は -5.9mm まで出る。圧縮側を 2 回掃引している形)、"
                + "(2) 記録した点列が並んでおらず折り返しが 30 回ある (図にすると往復する)。"
                + "出すなら先にこの 2 つを直す必要がある。出すか消すかは未決",
