...
namespace PileDesign.FEM
{
    ...
    public class VerticalLoadTransferMethod : BaseModel
    {
        ...
        public Vector<double>? GetDisplacementForGivenLoad(double pileTopForce)
        {
            ...
            // 範囲内のはずなのに、どの区間にも入らなかった。軸力が数値でない (NaN) と
            // 比較がすべて偽になるのでここへ来る。上の 2 つの範囲外チェックも同じ理由で
            // 素通りする。呼び出し側は null を受けて「何もしない」ので、沈下量は前回の値が
            // そのまま残る。黙って古い値が残るのが一番わかりにくいので、知らせる
            {
                string note = double.IsFinite(pileTopForce)
                    ? $"軸力 {pileTopForce:F0} kN に対する沈下量を荷重-沈下曲線から求められませんでした。沈下量を更新できませんでした。"
                    : "軸力が数値でないため、沈下量を荷重-沈下曲線から求められませんでした。沈下量を更新できませんでした。";
                if (!Warnings.Contains(note)) Warnings.Add(note);
            }
            return null;
        }
