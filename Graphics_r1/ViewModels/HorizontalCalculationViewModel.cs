...
namespace PileDesign.ViewModels
{
    public partial class HorizontalCalculationViewModel : ObservableObject
    {
        ...
        /// <summary>
        /// 杭の M–φ の接線剛性に、二方向曲げの連成 (合成 M–φ 曲線の接線の非対角項) を入れるか。
        ///
        /// 入れないと、二方向に曲げを受けて塑性化した要素の接線が、曲率の向きに沿った剛性を実際より大きく見積もる
        /// (非対角が対角の 3〜6 割に達する)。内力は割線剛性で求めるので、変わるのは反復の進み方だけで、
        /// 収束した答えは変わらない。検証中のため既定は入れない (2026-09-26)。
        /// </summary>
        public bool UseBiaxialTangentCoupling { get; set; }
        ...
        /// <summary>
        /// 残差比 ‖R‖²/‖F‖² がこの値より大きい間は、修正 NR でも K を再利用せず毎反復組み直す (混成則)。
        ///
        /// <para>2026-09-12 の収束研究で、地盤変位を入れた液状化ケースが初回試行のステップ 1 で
        /// 必ず失敗していた。全杭頭が同時にひび割れ・降伏したあと、古い (硬い) 接線を 20 反復以上
        /// 使い続けて残差比 ~290 で停滞し、プラトー検知 (遠方では 20 反復間隔) より先に
        /// 「30 反復未改善 → 許容値の緩和」が発火して受理→再試行に回っていた。
        /// ステップを倍にしても同じ場所で失敗する (増分の大きさではない)。</para>
        ///
        /// <para>残差比 > 1e-2 の間だけ Full NR にすると、計算例9/10 の L1・L2 がすべて再試行なしで
        /// 収束し (総反復 200→64、316→158、231→126、393→139)、時間も短くなった。仕上げ域
        /// (残差比 ≦ 1e-2) は従来どおり K を再利用するので、大型モデルでの修正 NR の速さは保つ。
        /// 元々易しい例題の負担は +20% 以内。</para>
        /// </summary>
        internal const double FullNRAboveResidualRatio = 1e-2;
        ...
        {
            get => InputModel?.RestrainFoundationTorsion ?? false;
            set
            {
                if (InputModel != null && InputModel.RestrainFoundationTorsion != value)
                {
                    InputModel.RestrainFoundationTorsion = value;
                    OnPropertyChanged();
                }
            }
        }
        ...
        */
        ...
        private bool ValidateIncrementalCompatibility(out string reason)
        {
            ...
            if (prev.RestrainFoundationTorsion != RestrainFoundationTorsion)
                diffs.Add($"基礎のねじれ拘束 切替 ({prev.RestrainFoundationTorsion}→{RestrainFoundationTorsion})");
