                ...
                foreach (var pile in InputModel.PileLayoutItems)
                {
                    ...
                    if (rxy.Mode == RotationalSpringMode.CombinedXY)
                    {
                            ...
                            && rxy.CrackNx.HasValue && rxy.CrackNy.HasValue && rxy.CurveXY != null)
                        {
                            ...
                            // n 方向: kParallel (曲線の剛性)。直交方向: kPerp = kParallel × PERP_RATIO。
                            //
                            // 2026-09-26 に 5% → 100% (直交方向も同じ剛性) に変えた。以前は「特異行列防止」のための
                            // 小さな直交剛性 5% で、割線 (内力) にも効いていた。ひび割れ後の杭頭は直交方向にほぼ自由に回り、
                            // 回転がロックした向きから 30〜40° ずれた。ロックする向きは何ステップ目でひび割れを検出したかで決まる
                            // ので、答えが荷重の刻み方に依存した (計算例10 L1 液状化: 最大モーメントがステップ数で 0° 7.8%・
                            // 60° 5.1% 変わり、刻み方によっては杭頭がひび割れない解に落ちた)。ひび割れた RC 杭頭は向きによる差が
                            // 小さいので直交方向も同じ剛性とし、方向ロックとヒステリシスは除荷の扱い (n 方向の投影) にだけ使う。
                            // 変更後: ステップ依存 0° 1.2%・60° 1.9%、標準例題の最大モーメントは −1.1〜+0.3%、検定の OK/NG は不変。
                            const double PERP_RATIO = 1.0;
