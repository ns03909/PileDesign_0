    ...
    class InsituSteelPipeReinforcedConcreteTopSection : AbstractPileSection
    {
        ...
        GetCrackMNInteraction(bool isLinear = false)
        {
            ...
            for (int i = 0; i <= div; i++)
            {
                ...
                else
                {
                    ...
                    epsTcr = InsituConcrete.GetCrackTensileStrain(MaterialLaw.Bilinear);
        ...
        internal (double, double) GetCrackMoment(double Ntarget, bool isLinear)
        {
            ...
            else
            {
                ...
                double epsTcr = InsituConcrete.GetCrackTensileStrain(MaterialLaw.Bilinear); // >0（引張ひずみの絶対値）
        ...
        private (bool hasYield, double My, double phiY) SolveYieldPoint2DNewton(double Ntarget)
            => SolveYieldPoint2DNewton(Ntarget,
                MainBars1.RSigmaY / MainBars1.Er,   // 引張降伏ひずみ (絶対値)
                PileDia * 0.5 + MainBarPCD1 * 0.5);  // 圧縮縁から主筋重心までの距離
